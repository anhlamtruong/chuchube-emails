...
import type { Recruiter, ClipboardPreviewRow } from "@/api/client";
...
export default function RecruitersPage() {
  ...
  const load = useCallback(async () => {
    ...
    const { items, total: t } = await getRecruiters(params);
    setRecruiters(items);
    setTotal(t);
    setPage(1);
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      if (debouncedLocation) params.location = debouncedLocation;
      if (debouncedTitle) params.title = debouncedTitle;
      const { items } = await getRecruiters(params);
      setRecruiters((prev) => [...prev, ...items]);
      setPage(nextPage);
    } finally {
  ...
  const handleSave = async () => {
    try {
      if (editId) {
        await updateRecruiter(editId, form);
      ...
      toast.error(err.response?.data?.detail || "Error saving recruiter");
    }
  };
  ...
  const handleDelete = async () => {
    ...
    await deleteRecruiter(deleteId);
  ...
  const handleImportFiles = async (files: FileList | File[]) => {
    ...
    try {
      ...
      toast.success(
        `Imported ${result.created} recruiters (${result.skipped} skipped)`,
      );
      load();
    } catch {
  ...
  const handlePasteConfirm = async (rows: ClipboardPreviewRow[]) => {
    try {
      const result = await commitClipboard({ rows, target: "recruiters" });
      toast.success(
        `Created ${result.recruiters_created} recruiters (${result.recruiters_existing} already existed)`,
      );
      load();
    } catch {
      toast.error("Import failed");
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <h2 className="text-2xl font-bold tracking-tight">Recruiters</h2>
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editId ? "Edit" : "Add"} Recruiter</DialogTitle>
          </DialogHeader>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete the recruiter. This cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <TableBody>
              {recruiters.length === 0 ? (
                <TableRow>
                  ...
                  >
                    No recruiters found. Add one, import files, or paste CSV
                    data.
                  </TableCell>
                </TableRow>
              ) : (
        ...
        <div className="flex items-center justify-center gap-3 py-2">
          ...
          >
            {loadingMore ? (
              ...
              `Load More (${recruiters.length} / ${total})`
            )}
          </Button>
        </div>
      )}
      ...
      <ClipboardPasteModal
        ...
        title="Paste Recruiters from CSV"
      />
    </div>
  );
}
